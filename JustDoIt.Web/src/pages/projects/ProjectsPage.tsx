...
export default function ProjectsPage({ authToken }: Readonly<Props>) {
  ...
  const getImageURL = useCallback(async (imageURL: string): Promise<string> => {
  ...
  useEffect(() => {
    ...
    fetchProjects();
    onClose();
  }, [fetchCategories, fetchProjects, fetchStatuses, onClose, onOpen]);
  ...
  const renderCell = React.useCallback(
    (project: Project, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        ...
        case "status":
          let status;
          if (statuses) status = statuses?.find((x) => x.Id == cellValue).value;
          return (
            <Chip
              className="capitalize"
              color={status ? projectStatusColorMap[status] : "default"}
              size="sm"
              variant="flat"
            >
              {status || "error"}
