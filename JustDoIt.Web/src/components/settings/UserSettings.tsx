...
const UserSettings = () => {
  ...
  const imgFirebaseRef = ref(storage, user?.pictureUrl);
  ...
  const handleFirebaseUpload = useCallback(() => {
    const storageRef = ref(
      storage,
      `gs://task-manager-just-do-it.appspot.com/profile-photos/${
        parseJwt(authToken).sub
      }.jpeg`
    );
    uploadBytes(storageRef, imageInputRef.current.files[0]).then((snapshot) => {
  ...
  return (
    <div className="border p-2 flex flex-col">
      ...
      <LoadingSpinner onClose={onClose} isOpen={isOpen} onOpen={onOpen} />
    </div>
  );
};
